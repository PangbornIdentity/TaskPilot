...
public class StatsService(ApplicationDbContext context) : IStatsService
{
    public async Task<TaskStatsResponse> GetTaskStatsAsync(string userId, CancellationToken cancellationToken = default)
    {
        ...
        var totalActive = await context.Tasks
            .CountAsync(t => t.UserId == userId && t.Status != Shared.Enums.TaskStatus.Completed && t.Status != Shared.Enums.TaskStatus.Cancelled, cancellationToken);
        ...
        var overdue = await context.Tasks
            .CountAsync(t => t.UserId == userId && t.TargetDate < today
                && t.Status != Shared.Enums.TaskStatus.Completed && t.Status != Shared.Enums.TaskStatus.Cancelled, cancellationToken);

        var inProgress = await context.Tasks
            .CountAsync(t => t.UserId == userId && t.Status == Shared.Enums.TaskStatus.InProgress, cancellationToken);

        var blocked = await context.Tasks
            .CountAsync(t => t.UserId == userId && t.Status == Shared.Enums.TaskStatus.Blocked, cancellationToken);
    ...
    {
        var raw = await context.Tasks
            .Where(t => t.UserId == userId && t.Status != Shared.Enums.TaskStatus.Completed && t.Status != Shared.Enums.TaskStatus.Cancelled)
            .GroupBy(t => t.Type)
    ...
    private async Task<List<PriorityBreakdownData>> GetByPriorityAsync(string userId, CancellationToken ct)
    {
        ...
        return raw
            ...
            .Select(g => new PriorityBreakdownData(
                g.Key.ToString(),
                g.Count(t => t.Status == Shared.Enums.TaskStatus.NotStarted),
                g.Count(t => t.Status == Shared.Enums.TaskStatus.InProgress),
                g.Count(t => t.Status == Shared.Enums.TaskStatus.Blocked)))
            .ToList();
    }
